...
class ModelExport:
    ...
    def _export_torchscripts(self, model, verbose, path, enc_size=None):
        ...
        # [func]
        # dummy_input = torch.load('data/test_data_1x93x560.pt')
        if self.device == 'cuda':
            model = model.cuda()
            dummy_input = tuple([i.cuda() for i in dummy_input])
        def _save(_module, _name):
            import pdb; pdb.set_trace()
            _script = torch.jit.trace(_module, dummy_input)
            _script.save(_name)
        """
        ...
        print('FP16: {}'.format(self.fp16))
        print('EXPORT_FP16: {}'.format(self.export_fp16))
        if self.fp16 and hasattr(model, 'encoder'):
            ...
            for tmp_model in tmp_models:
                ...
                for key, value in state_dict.items():
                    if '.feed_forward.w_2.' in key:
                        state_dict[key] = value / self.fp16
        ...
        if hasattr(model, 'encoder'):
            _save(model.encoder, 'sub_encoder_fixfp16.pt')
        if hasattr(model, 'decoder'):
            _save(model.decoder, 'sub_decoder.pt')
        return
        """
        ...
        import torch_blade
        if hasattr(model, 'encoder'):
            model.encoder = torch.jit.load('z_models/sub_encoder_fp16_blade.pt')
            model(*dummy_input)
            _save(model, 'encoder_fp16_blade.pt')
        if hasattr(model, 'decoder'):
            model.decoder = torch.jit.load('z_models/sub_decoder_fp16_blade.pt')
            _save(model, 'decoder_fp16_blade.pt')
        return
        """
